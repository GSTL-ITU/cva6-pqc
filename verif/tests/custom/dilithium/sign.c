...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) 
{
  ...
  //print_str("CRYPTO_SIGN_KEYPAIR: Expand matrix\n");
  /* Expand matrix */
  polyvec_matrix_expand(mat, rho);
  
  //print_str("CRYPTO_SIGN_KEYPAIR: Sample short vectors s1 and s2\n");
  /* Sample short vectors s1 and s2 */
  polyvecl_uniform_eta(&s1, rhoprime, 0);
  polyveck_uniform_eta(&s2, rhoprime, L);

  //print_str("CRYPTO_SIGN_KEYPAIR: Matrix-vector multiplication\n");
  ...
  //print_str("CRYPTO_SIGN_KEYPAIR: Add error vector s2\n");
  /* Add error vector s2 */
  polyveck_add(&t1, &t1, &s2);

  //print_str("CRYPTO_SIGN_KEYPAIR: Extract t1 and write public key\n");
  /* Extract t1 and write public key */
  polyveck_caddq(&t1);
  polyveck_power2round(&t1, &t0, &t1);
  pack_pk(pk, rho, &t1);

  //print_str("CRYPTO_SIGN_KEYPAIR: Compute H(rho, t1) and write secret key\n");
